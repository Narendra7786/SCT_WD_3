...
function loadQuestion() {
  ...
  q.options.forEach((opt, index) => {
    ...
    input.name = "option";
    input.value = opt;
    input.id = `opt${index}`; // ✅ fixed template literal

    const label = document.createElement("label");
    label.htmlFor = `opt${index}`; // ✅ fixed template literal
...
function showResult() {
  ...
  resultEl.innerHTML = `You scored ${score} out of ${quizData.length}`; // ✅ fixed template literal
}
